...
def get_forecast(code, start_date='2004-01-01', end_date='2017-12-12'):
    ...
    def setValue(v):
        ...
        if pd.isnull(v) or v == 'None' or v == '--':
            ...
            lastvalue = v
            return lastvalue
    cursor = forecast.find({'code':str(code), 'date':{'$gte':'2004-01-01', '$lte': end_date}}).sort('date')
    df = pd.DataFrame(list(cursor))
    if df.empty:
        return df
    del df['_id']
...
def get_all(pool, period, start_date, factors=[], count=0, index=True, us_market = False, **args):
    ...
    for code in tqdm(code_list):         
        ...
        if not df_holdfund.empty:
            df = df.merge(df_holdfund, how='left', on ='date', suffixes=('', '_y'))
            drop_y(df)
        if not df_forecast.empty:
            df = df.merge(df_forecast, how='left', on ='date', suffixes=('', '_y'))
            drop_y(df)
...
if __name__ == '__main__':
    ...
    # print(re[1])
    # print(re[2])  
    # factor_list = ['date','总市值','市盈率','市净率','市现率','市销率','总资产收益率',\
    #         '净资产收益率','营业毛利率','税前利润率','营业利润率','净利润率',\
    #         '销售收入/平均总资产','总资本回报率','总负债/总资产','普通股权益/总资产',\
    #         '派息比率','资本支出/销售额','流动比率','负债/息税前营业利润',\
    #         '速动比率','每股净资产','摊薄每股收益','每股销售额','open', 'close',\
    #         'code', 'high', 'low']

    # opdata.get_all('usall','1m','2010-01',factor_list,us_market=True)  